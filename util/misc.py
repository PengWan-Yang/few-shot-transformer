...
def tensor_from_tensor_list(tensor_list: List[Tensor]):
    # TODO make this more general
    if tensor_list[0][0].ndim == 4:
        tensor_shape = tensor_list[0][0].shape
        batch_shape = [len(tensor_list), len(tensor_list[0]), tensor_shape[0], tensor_shape[1],
                       tensor_shape[2], tensor_shape[3]]
        dtype = tensor_list[0][0].dtype
        device = tensor_list[0][0].device
        tensor = torch.zeros(batch_shape, dtype=dtype, device=device)
        for imgs, pad_imgs in zip(tensor_list, tensor):
